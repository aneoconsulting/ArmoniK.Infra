...
{{- define "armonik.compute.init.container" -}}
{{- $init := .root.Values.init -}}
name: init
...
{{- define "armonik.compute.init.podSpec" -}}
{{- $root := .root -}}
{{- $v := $root.Values -}}
{{- $init := $v.init -}}
{{- $global := $v.global | default dict -}}
{{- $container := list (include "armonik.compute.init.container" .) $init.containerPatch "init.containerPatch" | include "armonik.utils.patch" | fromYaml -}}
