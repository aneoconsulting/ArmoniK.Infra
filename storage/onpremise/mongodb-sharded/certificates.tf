...
resource "tls_cert_request" "mongodb_cert_request" {
  ...
  dns_names = [
    local.mongodb_dns,                                    # mongodb-sharded.armonik.svc.cluster.local
    var.name,                                            # mongodb-sharded
    "${var.name}.${var.namespace}",                      # mongodb-sharded.armonik
    "${var.name}.${var.namespace}.svc",                  # mongodb-sharded.armonik.svc
    "localhost"
  ]
