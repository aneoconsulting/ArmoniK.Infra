locals {
  ...
  control_plane_endpoints = (local.control_plane_load_balancer.ip == "" && kubernetes_service.control_plane.spec.0.type == "ClusterIP" ? {
    ...
    port = kubernetes_service.control_plane.spec.0.port.0.port
    service_dns = "${kubernetes_service.control_plane.metadata[0].name}.${kubernetes_service.control_plane.metadata[0].namespace}"    
    } : {
  ...
  admin_gui_endpoints = length(kubernetes_service.admin_gui) > 0 ? (local.admin_gui_load_balancer.ip == "" && kubernetes_service.admin_gui[0].spec.0.type == "ClusterIP" ? {
    ...
    app_port = kubernetes_service.admin_gui[0].spec.0.port.0.port
    service_dns = "${kubernetes_service.admin_gui[0].metadata[0].name}.${kubernetes_service.admin_gui[0].metadata[0].namespace}"
    } : {
  ...
  admin_old_gui_endpoints = length(kubernetes_service.admin_old_gui) > 0 ? (local.admin_old_gui_load_balancer.ip == "" && kubernetes_service.admin_old_gui[0].spec.0.type == "ClusterIP" ? {
    ...
    api_port = kubernetes_service.admin_old_gui[0].spec.0.port.0.port
    app_port = kubernetes_service.admin_old_gui[0].spec.0.port.1.port
    service_dns = "${kubernetes_service.admin_old_gui[0].metadata[0].name}.${kubernetes_service.admin_old_gui[0].metadata[0].namespace}"
    } : {
  ...
  admin_app_url     = length(kubernetes_service.admin_gui) > 0 ? "http://${local.admin_gui_endpoints.service_dns}:${local.admin_gui_endpoints.app_port}" : null
  admin_api_url     = length(kubernetes_service.admin_old_gui) > 0 ? "http://${local.admin_old_gui_endpoints.service_dns}:${local.admin_old_gui_endpoints.api_port}/api" : null
  admin_old_url     = length(kubernetes_service.admin_old_gui) > 0 ? "http://${local.admin_old_gui_endpoints.service_dns}:${local.admin_old_gui_endpoints.app_port}/" : null
