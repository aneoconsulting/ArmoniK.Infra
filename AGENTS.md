...
- Every chart sees its own direct subcharts in `.Subcharts`, but only the root sees them all, with every dependency's fully-coalesced, override-aware values; a subchart never sees its siblings. That is why ALL cross-chart derivation lives in the umbrella and is recomputed every render. `import-values` must never carry a dependency's values upward: it copies chart-file defaults only and silently drops every `-f`/`--set` override, which would deploy a customized backend whose connection env ArmoniK never receives.
...
**Stores.** A source reads the per-release Kubernetes-provider `SecretStore` unless it names one that ALREADY exists: `storeName` plus `storeKind` (`SecretStore`, which must then sit in the release namespace, or `ClusterSecretStore`). That is how a value comes from GCP Secret Manager, Vault or any other ESO provider; the charts create no external store. `armonik.conf.sourceRef` resolves and validates the fields for `envFromSecret`, `envSecret` and `mountSecret` alike: `namespace` stays the Kubernetes provider's own routing and excludes `storeName`, `field` (and a mount `items[].field`) is optional behind a `storeName` since a store entry may hold a single value, and `version` requires one, the Kubernetes provider ignoring it. `envSecret` items are therefore `"<secret>"` or `{secret, storeName, storeKind, namespace, version}`, normalized by `armonik.conf.secretItem` in both `merge` and `resolve`. `armonik-ingress` carries the same selector for its URLs Secret (`urlsSecretStoreName`/`urlsSecretStoreKind`/`urlsSecretVersion`). Sync cadence comes from `armonik.conf.refreshInterval` and `armonik.conf.refreshPolicy`, read per chart off `.Values.conf.*` with no global (`1h0m0s` and ESO's own default; the ingress declares `1m0s`), and cannot be per store: one layer is one ExternalSecret that may mix stores.
...
**Credentials.** ESO is the channel for credentials, references and content-derived values (assembling a `mongodb://user:pass@host` URI, for instance). It reads Secrets only, so credentials never land in a ConfigMap. Bring-your-own credentials means pre-creating the backend Secret; GitOps consumers get a deterministic, `lookup`-free render. `charts/armonik/templates/secrets/secret-store.yaml` renders the per-release `SecretStore`s (Kubernetes provider) with a dedicated read-only ServiceAccount, Role and RoleBinding. An ExternalSecret can only reference a store in its own namespace, so every namespace holding one of ours (the conf layers', mongodb-exporter's) gets the same set, named by `armonik.conf.storeName` relative to that namespace: a base store reading it, plus one per other namespace read. Each namespace read gets one Role and one RoleBinding, bound to every ServiceAccount reading it.

**Naming is a convention between the umbrella and our own charts, never imposed on external charts.** We do not shadow third-party naming helpers, nor tpl-inject names into dependency values. The umbrella reads whatever names the dependency charts produce (through `.Subcharts`) and wires them into the ESO `remoteRef`s. The only names we own are our own (conf Secrets, the SecretStore), all release-derived, so multi-release installs and non-`armonik` release names stay safe. Hence no fixed `fullnameOverride` on a dependency: it collides between two releases (cluster-scoped RBAC even across namespaces), and reading the real name makes it unnecessary. A dependency value naming one of OUR resources may carry a release-derived `tpl` where that chart renders it (valkey's `auth.usersExistingSecret`). Where a consumer's scope cannot reach the dependency's values (a third-party chart's `tpl`), the umbrella fronts the dependency with a Service of its own, named from the release name alone: `armonik.seq.ingestionService`, which fluent-bit ships to. One exception remains, documented at the value: the mongodb-exporter Secret keeps a fixed name, that chart `tpl`-rendering neither `existingSecret.name` nor `volumes`.

**Umbrella `namespaceOverride` is a split install.** It moves ArmoniK's own resources, and the planes with it (`namespace-guard.yaml` requires the same value on both). A dependency moves only through its own namespace key, and one without such a key (valkey, seq, fluent-bit, mongodb-exporter) stays in the release namespace. Whatever pairs with a dependency (its Certificate, its ExternalSecret, its ServiceMonitor) lives in that dependency's real namespace, read through its chart's helpers, never `armonik.namespace` of the umbrella; every reference to it (URLs, conf env, NetworkPolicy peers) resolves the same way.
...
`charts/armonik/templates/operators-guard.yaml` rejects `deploy=true` with `available=false`, and namespace drift between `global.armonik.operators.<op>.namespace` and an operator subchart's own `namespaceOverride`. When this release installs cert-manager, `charts/armonik/values.yaml` re-hooks cert-manager's own `startupapicheck` Job (`post-install,post-upgrade`, weight `-1`) as the webhook-readiness gate ahead of the Issuers and Certificates this release emits.

### Monitoring and autoscaling

`_monitoring.tpl` resolves `global.armonik.monitoring.{prometheusUrl,metricsExporterUrl}` and the Grafana dashboard sidecar's watched namespaces. `prometheusUrl` derives from kube-prometheus-stack's own naming helpers, so only where that chart is visible through `.Subcharts` (the umbrella or `armonik-operators` installing it); a layered application release and a plane chart's own context must state it, which the `armonik-operators` NOTES print. The default scaling path is one KEDA ScaledObject per partition with a `metrics-api` trigger scraping the control-plane metrics-exporter `/metrics` directly for `armonik_<partition>_tasks_queued`, which keeps Prometheus out of the scaling path; it assumes one always-present sample per metric, metrics-api having neither aggregation nor absent-as-zero. A `prometheus` trigger is the documented opt-in for rate, ratio and aggregation queries, and reintroduces the dependency on `prometheusOperator.available`. Prometheus gets ArmoniK metrics through first-party ServiceMonitor and PodMonitor resources in the plane charts, gated on `prometheusOperator.available`. A pod-deletion-cost controller (port of `armonik/pdc.tf`) is in target, so KEDA scale-in evicts idle workers rather than busy ones.
...
- **mongodb-exporter's Secret has a fixed name** (`dependencies.mongodb-exporter.existingSecret.name`, repeated in its `volumes`), so two umbrella releases in one namespace collide on it unless one renames it; documented at the value and in the umbrella NOTES. The chart tpl-renders neither value; the fix is upstream in prometheus-mongodb-exporter, after which both default to a release-derived name.
...
- `charts/airgap.md` documents installing from the packaged archives (`charts/package-charts.sh`) and from the OCI registry, including that `helm repo add` has no `file://` handler, so consuming the generated `index.yaml` means serving the directory over HTTP, and that it takes no OCI registry either, so the registry path has no index and needs an explicit `--version`. Container images are explicitly out of its scope.
...
- **Chart names are Docker Hub repository names.** `helm push` takes the OCI repository basename from the chart name and the tag from the chart version, and a Docker Hub repository is exactly `<namespace>/<name>` with no deeper path, so charts cannot be filed under a prefix: they share the `dockerhubaneo` namespace, and its tag namespace, with the ArmoniK images. Hyphenated `armonik-*` names there are reserved for charts, images keeping the `armonik_*` underscore form. Renaming a chart renames its repository, which a push creates silently.
