...
resource "kubernetes_secret" "mongodb_monitoring_connection_string" {
  ...
  data = {
    uri = "mongodb://${random_string.mongodb_monitoring_user.result}:${random_password.mongodb_monitoring_password.result}@${local.mongodb_dns}:27017/admin?tls=true&tlsAllowInvalidCertificates=true&tlsAllowInvalidHostnames=true&tlsCAFile=/mongodb/certs/ca.pem&authSource=admin"//&directConnection=true"
    # uri = "mongodb://${random_string.mongodb_monitoring_user.result}:${random_password.mongodb_monitoring_password.result}@${local.mongodb_dns}:27017/admin?authSource=admin&tls=true&tlsInsecure=true"
