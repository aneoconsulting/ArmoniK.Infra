...
variable "name" {
  description = "Name that serves as prefix for Kubernetes resources names"
  type        = string
  default     = ""
  validation {
    error_message = "Length of 'var.name' must not exceed 20 characters"
    condition     = length(var.name) > 20 ? false : true
  }
}
...
variable "mtls" {
  ...
  #   condition     = can(coalesce(var.nginx.client_certs.custom_ca_file)) && var.nginx.client_certs.generate ? false : true
  # }
  validation {
    ...
    condition     = var.mtls != null ? length(var.mtls.generate_certs_for) > 0 || length(var.mtls.extra_ca_paths) > 0 : true
  }
}
...
variable "gui" {
  ...
  validation {
    ...
    condition     = can(coalesce(var.gui.langs)) ? contains(var.gui.langs, "en") : true
  }
}
...
variable "nginx" {
  ...
  #   condition     = can(coalesce(var.load_balancer)) ? true : can(coalesce(var.nginx.underlying_endpoint))
  # }
}
...
variable "default_cluster" {
  ...
  #   condition     = can(coalesce(var.default_cluster)) ? contains(keys(var.clusters), var.default_cluster) : true
  # }
  # validation {
  #   error_message = "var.default_cluster is irrelevant if load balancing is not enforced (i.e. if var.load_balancer not null)"
  #   condition     = can(coalesce(var.default_cluster)) ? var.load_balancer != null : true
  # }
}
