...
`selfSigned`, same as always, except for postgresql, which bootstraps a CA instead; set it per
consumer for a different backend:
...
dependencies:
  postgresql:
    ...
    certManager:
      enabled: true
      # no issuer configured: a CA is bootstrapped in-cluster, see below
    cluster:
      certificates:
...
**With no issuer configured, a CA is bootstrapped in-cluster** instead of the selfSigned local
fallback the other consumers default to, the same chain the psmdb operator builds for MongoDB on
its own: a `<cluster>-selfsigned-issuer` Issuer signs the `<cluster>-ca-cert` CA Certificate (key
generated by cert-manager, valid 10 years), and the local `<cluster>-issuer` is a `ca` Issuer over
its Secret. Nothing has to exist beforehand. `-ca-cert` keeps clear of `<cluster>-ca`, the
operator's own CA Secret.

**An explicit selfSigned or acme fails at render time**, as `certManager.provider` or as the
shared issuer's provider. Unlike MongoDB, selfSigned would work: CNPG validates each half against
its own Secret. It is refused because every certificate would sign itself, which gains nothing over
the operator's own CA. acme only issues for DNS/IP identifiers, so it cannot issue
`streaming_replica`.
...
shared issuer, that means anyone allowed to request certificates from it. The bootstrapped CA, or
an issuer of PostgreSQL's own (`certManager.provider` or `existingIssuer`), keeps it to this
cluster.
