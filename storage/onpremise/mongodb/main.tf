  ...
  values = [
    yamlencode({
      ...
      "tls" = {
        "enabled"       = "true"
        "autoGenerated" = "false"
        # "caKey" = "/mongodb/certs/ca.pem"
        # # "certKey" = "/mongodb/certs/mongodb.pem"
        # "caCert" = "/mongodb/certs/mongodb.pem"
        "extraDnsNames"      = ["${var.name}-headless.${var.namespace}.svc.cluster.local"]
        "securityContext" = {
      ...
      "extraVolumes" = [
        {
          "name" = "mongodb-tls"
          "secret" = {
            "secretName" = "${var.name}-server-certificates"
          }
        }
      ]
      "extraVolumeMounts" = [
