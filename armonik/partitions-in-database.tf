...
locals {
  ...
  #!/bin/bash
  # Drop
  mongosh --tlsCAFile "$MongoDB__CAFile" --tlsAllowInvalidCertificates --tlsAllowInvalidHostnames --tls --username "$MongoDB__User" --password "$MongoDB__Password" "mongodb://$MongoDB__Host:$MongoDB__Port/$MongoDB__DatabaseName?authSource=admin" --eval 'db.PartitionData.drop()'
  # Insert

  mongosh --tlsCAFile "$MongoDB__CAFile" --tlsAllowInvalidCertificates --tlsAllowInvalidHostnames --tls --username "$MongoDB__User" --password "$MongoDB__Password" "mongodb://$MongoDB__Host:$MongoDB__Port/$MongoDB__DatabaseName?authSource=admin" --eval 'db.PartitionData.insertMany(${jsonencode(local.partitions_data)})'
  EOF
}
