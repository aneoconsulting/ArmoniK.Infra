...
locals {
  ...
  kubernetes        = coalesce(
    length(module.gke) > 0 ? module.gke[0] : null,
    length(module.private_gke) > 0 ? module.private_gke[0] : null,
    length(module.autopilot) > 0 ? module.autopilot[0] : null,
    length(module.private_autopilot) > 0 ? module.private_autopilot[0] : null
  )
}
...
resource "null_resource" "update_kubeconfig" {
  ...
  provisioner "local-exec" {
    command     = "gcloud container clusters get-credentials ${local.kubernetes.name} --location=${local.kubernetes.location}"
    environment = {
