...
# partitions-guard makes a partition-less compute-plane a render error, so the
...
# The chart carries no defaults of its own (all configuration is done in the parent chart); set here
# what the umbrella defaults to (see charts/armonik/values.yaml): seq >=2026.x fails the render unless
# one first-run auth option is set, and postgresql, left unstated, renders a Cluster with null
# spec.postgresql and spec.bootstrap.initdb.
...
fail_with compute-patch-builds-list armonik-compute-plane \
  "podSpecPatch.containers: the chart builds containers" \
  -f test/fixtures/armonik-compute-plane/patch-list-values.yaml
fail_with control-patch-builds-list armonik-control-plane \
  "containerPatch.env: the chart builds env" \
  -f test/fixtures/armonik-control-plane/patch-list-values.yaml
